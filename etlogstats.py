	...
	def offlineStats(self):
		...
		for subdir, dirs, files in os.walk(rootdir):
			for file in files:
				...
				# Get the instition name
				longPath = subdir.split('/')
				institution = longPath[2]

				if institution not in institutionCount:
					institutionCount[institution] = 0
					...
					if hash not in hashs:
						...
						with open(filepath) as fp:
							for line in fp:
								...
								if line.startswith('#') or '2009' in line:
									# Header of log file
									pass
								else:
									# Log line
									...
									if "pdf" in line or "zip" in line or "mp4" in line or "swf" in line or "flv" in line:
									...
									if '200' in line and not any(x in line for x in ('.zip', '.pdf', '.css', '.js', '.png', '.jpg', '.jpeg', '.gif', '.mp4', '.ico', '.swf', '.flv')) and '.html' in line:
									...
									if 'swf' in line:
										swfDownloads = swfDownloads+1

									if "pdf" in line:
										if "britishcouncil" in line:
											britishCouncil = britishCouncil+1

											# Tally the actual resource
											lineParts = line.split(' ')
											pathPart = lineParts[4].split('/')
											...
											# If the key exists in the dictionary, increment, else add it
											if '.pdf' in bcFile:
												if bcFile in bcFiles:
													bcFiles[bcFile] = bcFiles[bcFile]+1
												else:
													bcFiles[bcFile] = 1
		...
		print('British Council PDF downloads: '+str(britishCouncil))

		print('Downloads by month:')
		#pp = pprint.PrettyPrinter(indent=3)
		#pp.pprint(monthTotals)

		# British Council Tally
		print('British Council tally:')
		for key, value in bcFiles.items():
			print(key, value)

		# Institution tally
		print('Institution tally:')
		for key, value in institutionCount.items():
			print(key, value)
		...
		with open('offlinestats.csv', 'w', newline='') as csvfile:
			...
			onlineCSV.writerow(['Offline Stats'])
			onlineCSV.writerow([''])
			...
			onlineCSV.writerow(['Total downloads (PDF, ZIP, MP4, SWF)', downloadTotal])
			onlineCSV.writerow(['Video downloads (MP4)', videoDownloads])
			onlineCSV.writerow(['Flash downloads (SWF)', swfDownloads])
			onlineCSV.writerow([''])

			onlineCSV.writerow(['British Council PDF downloads'])
			onlineCSV.writerow(['Filename', 'Downloads'])
			for key, value in bcFiles.items():
				onlineCSV.writerow([key, value])
			onlineCSV.writerow([''])

			onlineCSV.writerow(['Institution downloads'])
			onlineCSV.writerow(['Institution', 'Downloads'])
			for key, value in institutionCount.items():
				onlineCSV.writerow([key, value])
			onlineCSV.writerow([''])
